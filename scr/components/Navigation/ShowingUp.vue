  ...
  <div>
    <div class="title" v-show="flag">{{ title }}</div>
      <ul class="mui-table-view">
        <li class="mui-table-view-cell mui-media" v-for="item in ShowingUpList" :key="item.id">
...
export default {
  ...
  methods: {
    getLists() {
      this.$http
        .jsonp("https://douban.uieee.com/v2/movie/in_theaters?city=梅州")
        .then(
          res => {
            if (res.status === 200) {
              this.ShowingUpList = res.body.subjects;
